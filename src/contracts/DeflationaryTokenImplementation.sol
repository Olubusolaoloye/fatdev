...
/**
 * @title DeflationaryTokenImplementation
 * @dev ERC-20 deflationary token — same battle-tested tax architecture as TaxToken,
 ...
 */
contract DeflationaryTokenImplementation is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256 private _totalBurned;                   // Cumulative tokens burned via tax deflation
    ...
    */
    ...
    function updateTaxDistribution(TaxDistribution memory newDistribution) external onlyOwner {
        uint256 totalDistribution = newDistribution.marketingPercent +
                                     ...
                                     newDistribution.deflationPercent;
    ...
    // ============================================
    // VIEW FUNCTIONS - DEFLATIONARY
    // ============================================

    /// @notice Cumulative tokens burned via tax-driven deflation
    function totalBurned() external view returns (uint256) {
        return _totalBurned;
    }

    /// @notice Preview the burn portion of a transfer
    /// @return burnAmount Tokens that would be burned
    /// @return netAmount Tokens recipient would receive after tax
    function calculateBurn(address from, address to, uint256 amount)
        external
        view
        returns (uint256 burnAmount, uint256 netAmount)
    {
        (uint256 totalTax, uint256 afterTax,) = this.calculateTax(from, to, amount);
    ...
    /// @notice True if any tax route is active and deflationPercent > 0
    ...
    /// @notice Total supply minus tokens held by contract and burn address
    ...
    /// @notice Aggregate burn metrics for dashboards
    /// @return totalBurned_ Cumulative burned amount
    /// @return burnPercentage Percent of original supply burned (basis points)
    /// @return circulatingSupply Tokens actually circulating
    function getBurnStatistics()
        external
        view
        returns (uint256 totalBurned_, uint256 burnPercentage, uint256 circulatingSupply)
    {
