...
type Section     = { title: string; icon: IconName; shareIcon: string; checks: CheckResult[] }
...
export function AuditScore() {
  ...
  const sections: Section[] = [
    ...
    {
      title: 'Security Flags', icon: 'lock', shareIcon: '🔒',
      checks: [
        {
    ...
    {
      title: 'Token Setup', icon: 'settings', shareIcon: '⚙️',
      checks: [
        {
    ...
    {
      title: 'Post-Deploy Status', icon: 'rocket', shareIcon: '🚀',
      checks: [
        {
  ...
  function copyShareText() {
    ...
    const text = [
      ...
      '',
      ...sections.map(s => `${s.shareIcon} ${s.title}\n` + s.checks.map(c => `  ${c.pass ? '✅' : c.warn ? '⚠️' : '❌'} ${c.label}`).join('\n')),
      '',
      contractAddr ? `Contract: ${contractAddr}` : '',
      `\nGenerated by FatDev — fatdev.org`,
    ].filter(Boolean).join('\n')
    ...
    try {
      generateAuditPdf({
        ...
        sections: sections.map(s => ({
          title:  s.title,   // PDF renders its own layout; no icon glyph needed
          checks: s.checks,
        })),
        onChain: onChainData,
      })
  ...
  return (
    <div className="step-panel" id="audit-printable" ref={printRef}>
        ...
        {onChainData ? (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <span className={`pill ${onChainData.verified ? 'pill-ok' : 'pill-warn'}`}>
              <Icon name={onChainData.verified ? 'check' : 'x'} size={12} />{onChainData.verified ? 'Verified' : 'Unverified'}
            </span>
            <span className={`pill ${onChainData.ownerRenounced ? 'pill-ok' : 'pill-warn'}`}>
              <Icon name={onChainData.ownerRenounced ? 'check' : 'alert'} size={12} />{onChainData.ownerRenounced ? 'Renounced' : 'Owner active'}
            </span>
            <span className={`pill ${onChainData.hasLiquidity ? 'pill-ok' : 'pill-warn'}`}>
              <Icon name={onChainData.hasLiquidity ? 'check' : 'alert'} size={12} />{onChainData.hasLiquidity ? 'Liquidity' : 'No liquidity'}
            </span>
          ...
          <button className="btn-ghost" style={{ fontSize: 12, padding: '6px 14px' }}
            ...
            {checking ? <Spinner /> : <><Icon name="search" size={14} />Run checks</>}
          </button>
        )}
            ...
            <div key={section.title} className="card" style={{ padding: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <span style={{ fontWeight: 700, fontSize: 13, display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                  <Icon name={section.icon} size={15} style={{ color: 'var(--fd-cyan)' }} />
                  {section.title}
                </span>
                ...
                {section.checks.map(check => (
                  <div key={check.label} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                    <span style={{ fontSize: 14, flexShrink: 0, marginTop: 1 }}>
                      <Icon name={check.pass ? 'check' : check.warn ? 'alert' : 'x'} size={14}
                        style={{ color: check.pass ? 'var(--fd-green)' : check.warn ? 'var(--amber)' : 'var(--red)' }} />
                    </span>
                    <div style={{ flex: 1 }}>
      ...
      {/* Disclaimer */}
      <div style={{ padding: '11px 16px', borderRadius: 8, background: 'rgba(255,176,32,0.06)',
        border: '1px solid rgba(255,176,32,0.22)', fontSize: 11.5, color: 'var(--text-muted)',
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        <button className="btn-primary" onClick={downloadPdf} disabled={pdfBusy}
          ...
          {pdfBusy ? <Spinner /> : <><Icon name="file" size={15} />Download PDF Report</>}
        </button>
        <button className="btn-ghost" onClick={copyShareText}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 7 }}>
          <Icon name="copy" size={15} />Copy for Telegram / Discord</button>
        {contractAddr && (
